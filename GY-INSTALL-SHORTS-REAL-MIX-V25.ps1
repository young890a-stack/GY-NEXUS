param(
  [string]$TargetPath = "",
  [string]$ZipPath = "$HOME\Downloads\GY-NEXUS-SHOPPING-SHORTS-REAL-MIX-V2.5.zip",
  [switch]$SkipBuild
)
...
function Copy-Tree([string]$Source, [string]$Destination) {
  ...
  $code = $LASTEXITCODE
  if ($code -gt 7) { throw "File copy failed. Robocopy exit code: $code" }
}
...
if ($target -eq $homePath -or $target -eq [IO.Path]::GetPathRoot($target)) { throw "Unsafe TargetPath." }
if (-not (Test-Path -LiteralPath (Join-Path $target "package.json"))) { throw "GY-NEXUS was not found. package.json is missing: $target" }

if (-not (Test-Path -LiteralPath $ZipPath)) {
  $zip = Get-ChildItem -LiteralPath "$HOME\Downloads" -Filter "GY-NEXUS-SHOPPING-SHORTS-REAL-MIX-V2.5*.zip" -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1
  if ($null -eq $zip) { throw "The V2.5 ZIP was not found in Downloads." }
  $ZipPath = $zip.FullName
}

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backup = Join-Path "$HOME\Documents" "GY-NEXUS-source-backup-v25-$stamp"
$temp = Join-Path ([IO.Path]::GetTempPath()) "gy-nexus-v25-$stamp"
...
try {
  ...
  Step "Overlaying Shopping Shorts Real Mix V2.5"
  Copy-Tree $source $target

  $connectorSource = Join-Path $source "browser-extension\gy-china-account-connector"
  if (Test-Path -LiteralPath (Join-Path $connectorSource "manifest.json")) {
    Step "Updating the Edge account connector"
  ...
  Write-Host "IMPORTANT: deploy render.yaml and connect VIDEO_WORKER_URL before real MP4 mixing." -ForegroundColor Yellow
}
catch {
  ...
  if (Test-Path -LiteralPath (Join-Path $backup "package.json")) { Write-Host "Source backup: $backup" -ForegroundColor Yellow }
  throw
}
finally {
  if (Test-Path -LiteralPath $backupBuildInfo) { Copy-Item -LiteralPath $backupBuildInfo -Destination $buildInfo -Force }
