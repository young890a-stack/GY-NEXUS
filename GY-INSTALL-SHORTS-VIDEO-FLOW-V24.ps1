param(
  [string]$TargetPath = "",
  [string]$ZipPath = "$HOME\Downloads\GY-NEXUS-SHOPPING-SHORTS-VIDEO-FLOW-V2.4.zip",
  [switch]$SkipBuild
)
...
if (-not (Test-Path -LiteralPath $ZipPath)) {
  $zip = Get-ChildItem -LiteralPath "$HOME\Downloads" -Filter "GY-NEXUS-SHOPPING-SHORTS-VIDEO-FLOW-V2.4*.zip" -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1
  if ($null -eq $zip) { throw "The V2.4 ZIP was not found in Downloads." }
  $ZipPath = $zip.FullName
}

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backup = Join-Path "$HOME\Documents" "GY-NEXUS-source-backup-v24-$stamp"
$temp = Join-Path ([IO.Path]::GetTempPath()) "gy-nexus-v24-$stamp"
...
try {
  ...
  Step "Overlaying Shopping Shorts Video Flow V2.4"
