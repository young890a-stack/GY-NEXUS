...
async function download(url, destination, expectedKind = "video") {
  ...
  if (expectedKind === "video" && !contentType.startsWith("video/") && !contentType.includes("octet-stream")) {
    throw new Error("직접 재생 가능한 영상 파일 주소가 아닙니다.");
  }
  if (expectedKind === "audio" && !contentType.startsWith("audio/") && !contentType.includes("octet-stream")) {
    throw new Error("직접 재생 가능한 음원 파일 주소가 아닙니다.");
  }
...
function run(command, args, cwd) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => code === 0
      ? resolve()
      : reject(new Error(`${command} 실패 (${code}): ${errorText.slice(-3000)}`)));
  });
}
...
async function uploadOutput(filePath, projectId) {
  ...
  const objectPath = `videos/${new Date().toISOString().slice(0, 10)}/${Date.now()}-gy-revenue-shorts-${String(projectId).replace(/[^a-z0-9-]/gi, "").slice(0, 40)}.mp4`;
...
async function renderJob(jobId, body) {
  const root = await fs.mkdtemp(join(tmpdir(), `gy-revenue-${jobId}-`));
  let stage = "작업 준비";
  activeJobId = jobId;
  ...
  try {
    ...
    const sourceAudioMode = String(settings.sourceAudioMode || "mute");
    ...
    const sources = await mapWithConcurrency(plan, 2, async (planItem, index) => {
      ...
      await run("ffmpeg", [
        "-y", ...seekArgs, "-i", sourcePath,
        "-t", String(sourceTake),
        "-vf", videoFilter,
        "-an", "-r", "30",
        "-c:v", "libx264", "-preset", "veryfast", "-crf", "19",
        "-pix_fmt", "yuv420p", normalizedPath,
      ], root);
    ...
    if (settings.voiceAudioUrl && sourceAudioMode === "mute-korean-tts") {
      const voicePath = join(root, "voice-audio");
      await download(settings.voiceAudioUrl, voicePath, "audio");
      inputs.push("-i", voicePath);
      audioFilters.push(`[${inputIndex}:a]volume=1.0,apad=pad_dur=${totalDuration}[voice]`);
    ...
    const bgmUrl = String(settings.customMusicUrl || process.env[`VIDEO_BGM_${moodKey}_URL`] || "");
    if (bgmUrl && moodKey !== "NONE") {
      const musicPath = join(root, "music-audio");
      await download(bgmUrl, musicPath, "audio");
      inputs.push("-stream_loop", "-1", "-i", musicPath);
      const musicVolume = Math.max(0, Math.min(.5, Number(settings.musicVolume) || .1));
      audioFilters.push(`[${inputIndex}:a]volume=${musicVolume},apad=pad_dur=${totalDuration}[music]`);
    ...
    if (audioLabels.length > 1) {
      audioFilters.push(`${audioLabels.join("")}amix=inputs=${audioLabels.length}:duration=longest:normalize=0[mixed]`);
    }
    const escapedAss = assPath.replace(/\\/g, "/").replace(/:/g, "\\:").replace(/'/g, "\\'");
    const filter = [`[0:v]ass='${escapedAss}'[video]`, ...audioFilters].join(";");
    await run("ffmpeg", [
      "-y", ...inputs,
      "-filter_complex", filter,
      "-map", "[video]", "-map", mixedLabel,
...
const server = createServer(async (request, response) => {
  if (request.method === "GET" && request.url === "/health") {
    return send(response, 200, { ok: true, service: "gy-nexus-video-worker", version: "revenue-shorts-1.0.0", activeJobId, queuedJobs });
  }
...
server.listen(PORT, "0.0.0.0", () => {
  process.stdout.write(`GY-NEXUS Revenue Shorts Worker listening on ${PORT}\n`);
});
