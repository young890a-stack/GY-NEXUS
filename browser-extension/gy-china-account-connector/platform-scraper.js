...
function cardFor(anchor) {
  ...
  for (let depth = 0; depth < 6 && current?.parentElement; depth += 1) {
    ...
    if (textLength >= 8 && textLength <= 1800 && rect.width >= 120 && rect.height >= 150) return current;
  }
  return anchor.parentElement || anchor;
}

function titleFor(anchor, card, platform) {
  const direct = anchor.getAttribute("title") || anchor.getAttribute("aria-label") || card.querySelector("[title]")?.getAttribute("title") || "";
...
function thumbnailFor(card) {
  const image = Array.from(card.querySelectorAll("img")).find((item) => {
    ...
    return rect.width >= 80 && rect.height >= 100;
  });
...
function normalizePageUrl(value, platform) {
  try {
    ...
    const valid = platform === "douyin"
      ...
      : /(^|\.)xiaohongshu\.com$/.test(url.hostname) && /\/(explore|discovery\/item)\//.test(url.pathname);
    if (!valid) return "";
    url.hash = "";
    return url.toString();
  } catch {
...
async function scrapeVisibleShorts(platform, limit) {
  window.scrollTo(0, Math.min(document.documentElement.scrollHeight * 0.45, 1400));
  await delay(900);
  const selector = platform === "douyin" ? 'a[href*="/video/"]' : 'a[href*="/explore/"],a[href*="/discovery/item/"]';
  ...
  const loginRequired = deduplicated.size === 0 && /(登录|扫码登录|手机号登录)/.test(bodyText);
  const results = Array.from(deduplicated.values());
  return {
    results,
    keywords: results.flatMap((item) => item.hashtags),
    loginRequired,
    message: results.length ? `${results.length} visible short-form cards found.` : "No short-form cards found on the visible search page.",
  };
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  scrapeVisibleShorts(platform, limit)
    .then(sendResponse)
    .catch((error) => sendResponse({ results: [], keywords: [], loginRequired: false, message: error instanceof Error ? error.message : "Scrape failed." }));
  return true;
});
