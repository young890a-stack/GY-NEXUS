...
export async function analyzeReferenceMaterial(input: {
  ...
  const parsed = await withStructuredJsonRetry<Omit<ReferenceAnalysis, "analyzedAt" | "model">>({
    label: "참고영상 장면 분석",
    request: async (attempt) => {
      const response = await openai.responses.create({
        ...
        max_output_tokens: attempt === 0 ? 5200 : 7600,
      });
      return response.output_text;
    },
    validate: (value) => Boolean(value && Array.isArray(value.sceneDecisions) && Array.isArray(value.mixPlan)),
  });
...
export async function generateSelectedSourceMix(input: {
  ...
  const parsed = await withStructuredJsonRetry<{
    title: string;
    cuts: SourceMixPlan["cuts"];
    safetySummary: string;
  }>({
    label: "선택 소스 AI 믹스",
    request: async (attempt) => {
      const response = await openai.responses.create({
        ...
        max_output_tokens: attempt === 0 ? 5600 : 8200,
      });
      return response.output_text;
    },
    validate: (value) => Boolean(value && Array.isArray(value.cuts)),
  });
...
export async function generateTrendIntelligence(input: {
  ...
  const model = process.env.OPENAI_FAST_MODEL || process.env.OPENAI_STRATEGY_MODEL || process.env.OPENAI_QUALITY_MODEL || "gpt-5.6-sol";
  const parsed = await withStructuredJsonRetry<
    Omit<TrendIntelligence, "discoveryLinks" | "referenceRule" | "generatedAt" | "model">
  >({
    label: "중국 트렌드 참고 설계",
    request: async (attempt) => {
      const response = await openai.responses.create({
        ...
        max_output_tokens: attempt === 0 ? 4800 : 6800,
      });
      return response.output_text;
    },
    validate: (value) => Boolean(value && Array.isArray(value.chineseKeywords)),
  });
