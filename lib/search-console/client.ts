...
function credentials() {
  const clientId = (process.env.SEARCH_CONSOLE_CLIENT_ID || process.env.BLOGGER_CLIENT_ID || process.env.YOUTUBE_CLIENT_ID)?.trim();
  const clientSecret = (process.env.SEARCH_CONSOLE_CLIENT_SECRET || process.env.BLOGGER_CLIENT_SECRET || process.env.YOUTUBE_CLIENT_SECRET)?.trim();
  if (!clientId || !clientSecret) throw new Error("Google OAuth Client ID/Secret이 없습니다.");
