...
export default function GrowthCommerceEngine() {
  ...
  const [youtubeReconnectRequired, setYoutubeReconnectRequired] = useState(false);
  ...
  async function run(endpoint: string, key: string, method = "POST") {
    ...
    try {
      ...
      if (key === "youtube") {
        setYoutubeReconnectRequired(Boolean(data.reconnectRequired));
        if (data.videos?.length) {
          setStatus((current) => current ? {
            ...current,
            setupRequired: data.setupRequired ?? current.setupRequired,
            videos: data.videos || current.videos,
          } : current);
        }
      }
      const warning = data.warnings?.[0];
      setMessage([data.message || "완료했습니다.", warning].filter(Boolean).join(" "));
      if (!data.setupRequired) await load();
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.actions}>
      ...
      <article><span>YOUTUBE DATA</span><h2>성과·판매 통합 동기화</h2><p>조회수, 유지율, 참여, 추적 클릭, 전환, 매출을 영상별로 합칩니다. 데이터베이스 준비 전에도 기본 조회수는 먼저 확인합니다.</p><button onClick={() => void run("/api/growth-commerce/youtube/sync", "youtube")} disabled={busy === "youtube"}>{busy === "youtube" ? "동기화 중…" : "YouTube 성과 동기화"}</button>{(!status?.youtube?.connected || youtubeReconnectRequired) && <a href="/admin/connections">YouTube 한 번 다시 연결</a>}</article>
