...
export default function StrategyRoom() {
  ...
  return <div className="dy-strategy-layout">
    ...
    <section className="dy-meeting-board">
      ...
      <div className={`dy-verification ${result.verification.status}`}><div><span>AI CROSS-CHECK</span><h3>{result.verification.provider === "gemini" ? `Gemini 독립 검증 · ${result.verification.score ?? "-"}점` : "단일 모델 실행"}</h3><p>{result.verification.summary}</p></div><b>{result.verification.status === "verified" ? "검증 통과" : result.verification.status === "hold" ? "실행 보류" : result.verification.status === "revision_required" ? "수정 필요" : result.verification.status === "error" ? "검증 오류" : "미연결"}</b>{result.verification.corrections.length > 0 && <ul>{result.verification.corrections.map((item) => <li key={item}>{item}</li>)}</ul>}</div>
