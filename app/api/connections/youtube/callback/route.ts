...
export async function GET(request: NextRequest) {
  const fail = (reason: string) => {
    const response = NextResponse.redirect(connectionResultUrl(request, "youtube", "error", reason));
  ...
  try {
    const response = NextResponse.redirect(connectionResultUrl(request, "youtube", "connected"));
    response.cookies.set("gy_youtube_token", encryptConnectionValue({ ...token, created_at: Date.now() }), CONNECTION_COOKIE_OPTIONS);
    response.cookies.delete("gy_youtube_state");
    return response;
  } catch (error) {
    console.error("YouTube OAuth token storage failed", error);
    return fail("storage");
  }
}
