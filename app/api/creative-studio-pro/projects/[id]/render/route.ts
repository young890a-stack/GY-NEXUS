...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const licensedFinalAssets = normalizeMediaReferences(settings.mediaReferences).filter((item) => item.useInFinal && item.rightsStatus !== "unverified");
    ...
    const readyScenes = (scenes || []).filter((scene: any) => scene.status === "completed" && scene.video_url);
    ...
    const audioTimeline = record(settings.audioTimeline);
    const hasSegmentVoice = Array.isArray(audioTimeline.voiceSegments)
      && audioTimeline.voiceSegments.some((value) => String(record(value).audioUrl || "").startsWith("https://"));
    if (settings.sourceAudioMode === "mute-korean-tts" && !settings.voiceAudioUrl && !hasSegmentVoice) {
      return NextResponse.json({ success: false, message: "한국어 AI 음성을 먼저 생성해주세요." }, { status: 400 });
    }
    ...
    const response = await fetch(`${workerUrl}/render`, {
      ...
      body: JSON.stringify({ project: workerProject, scenes, callbackUrl: `${new URL(request.url).origin}/api/creative-studio-pro/projects/${id}/render-callback` }),
    });
    const result = await response.json().catch(() => ({})) as { message?: string; jobId?: string };
    ...
    return NextResponse.json({ success: false, message: error instanceof Error ? error.message : "렌더링 요청 실패" }, { status: 500 });
  }
}
