...
export async function GET() {
  try {
    ...
    const [runs, candidates, opportunities, variants] = await Promise.all([
      supabase.from("china_discovery_runs").select("*").order("created_at", { ascending: false }).limit(8),
      supabase.from("china_video_candidates").select("*").order("total_intelligence_score", { ascending: false }).limit(12),
    ...
    const firstError = [runs.error, candidates.error, opportunities.error, variants.error].find(Boolean);
