...
import { CoupangPartnersError, findCoupangProductById } from "@/lib/affiliate/coupang";
...
function coupangProductId(url: URL) {
  const pathMatch = url.pathname.match(/\/(?:vp\/)?products\/(\d+)/i);
  const queryValue = url.searchParams.get("productId") || url.searchParams.get("product_id");
  const value = pathMatch?.[1] || queryValue || "";
  return /^\d+$/.test(value) ? value : "";
}

function blockedPageReason(status: number, html: string) {
  if (status === 401 || status === 403) return `HTTP ${status}`;
  const title = titleText(html).toLowerCase();
  const sample = html.slice(0, 120_000).toLowerCase();
  if (/^(?:access denied|forbidden|request blocked|error)$/.test(title)) return title || "access denied";
  if (/(?:access denied|request blocked|the request could not be satisfied|generated by cloudfront|captcha|비정상적인 접근)/i.test(sample)) {
    return "access denied";
  }
  return "";
}

function importedFromCoupangApi(
  item: Awaited<ReturnType<typeof findCoupangProductById>>,
  requestedUrl: URL,
): ImportedProduct {
  return {
    name: item.title,
    ...
    imageStored: false,
    imageSource: item.imageUrl ? "remote" : "none",
    priceText: item.priceText,
    discountText: "",
    platform: "coupang",
    finalUrl: item.affiliateUrl || requestedUrl.toString(),
    resolvedUrl: requestedUrl.toString(),
    source: "coupang-api",
    ...(!item.imageUrl ? { warning: "쿠팡 API에서 상품은 찾았지만 대표 이미지가 없습니다. 직접 이미지를 업로드해주세요." } : {}),
  };
}
...
export async function POST(request: Request) {
  try {
    ...
    let coupangApiWarning = "";
    if (platformFor(requestedUrl) === "coupang") {
      const productId = coupangProductId(requestedUrl);
      if (productId) {
        try {
          const apiItem = await findCoupangProductById(productId);
          const apiProduct = importedFromCoupangApi(apiItem, requestedUrl);
          return NextResponse.json({
            success: true,
            product: await withPersistedImage(apiProduct, requestedUrl),
          });
        } catch (error) {
          coupangApiWarning = error instanceof CoupangPartnersError
            ? error.message
            : `쿠팡 API 상품 조회 실패: ${error instanceof Error ? error.message : "알 수 없는 오류"}`;
        }
      } else {
        coupangApiWarning = "쿠팡 주소에서 상품번호를 찾지 못했습니다.";
      }
    }

    const fetched = await fetchProductPage(requestedUrl);
    const blockedReason = blockedPageReason(fetched.response.status, fetched.html);
    if (blockedReason) {
      const blockedProduct: ImportedProduct = {
        name: "",
        description: "",
        imageUrl: "",
        originalImageUrl: "",
        ...
        priceText: "",
        discountText: "",
        platform: platformFor(fetched.finalUrl),
        finalUrl: requestedUrl.toString(),
        resolvedUrl: fetched.finalUrl.toString(),
        source: "link-only",
        warning: mergeWarning(
          `쿠팡이 서버 자동 읽기를 차단했습니다(${blockedReason}). 잘못된 'Access Denied' 정보는 입력하지 않았습니다.`,
          coupangApiWarning || "쿠팡파트너스 API 연결 상태를 확인하거나 상품 이미지를 직접 업로드해주세요.",
        ),
      };
      return NextResponse.json({ success: true, product: blockedProduct });
    }
    ...
    product.warning = mergeWarning(product.warning, coupangApiWarning) || undefined;
